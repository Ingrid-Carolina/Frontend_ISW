...
const EnVivo = () => {
	...
	const normalizeBool = v => {
		...
		if (typeof v === 'string') {
			...
			if (['true', 't', '1', 's', 'si', 'sí', 'y', 'yes', 'on'].includes(s))
				return true;
			if (['false', 'f', '0', 'n', 'no', 'off'].includes(s)) return false;
		}
		return !!v;
	};
	// arriba del componente, fuera de useEffect
		...
		try {
			const saved =
				typeof window !== 'undefined'
					? localStorage.getItem(ANUNCIO_KEY)
					: null;
	...
	const handleToggleAnuncio = async e => {
		...
		try {
			...
			// Asegura que el estado final sea el que guardó el server
			setMostrarAnuncio(!!data?.mostrar_anuncio);
		} catch (err) {
			setMostrarAnuncio(v => !v); // revertir
			showMessage(
				err?.response?.data?.mensaje ||
					'No se pudo guardar el estado del anuncio',
				'error',
			);
		}
	};
	...
	// en el Switch:
	<Switch checked={mostrarAnuncio} onChange={handleToggleAnuncio} />;
	...
	const fetchUrls = async () => {
		...
		try {
			...
			if (videosActualizados.length > 0) {
				...
				//toma el valor del backend (y también lo deja en localStorage useEffect)
	...
	const convertToEmbedUrl = (url, tipoTransmision = 'en_directo') => {
		...
		try {
			...
			if (videoId) {
				const autoplayParam =
					tipoTransmision === 'en_directo' ? '&autoplay=1&mute=1' : '';
			...
			>
				...
				<Box
					sx={{
						...
						alignItems: 'center',
						justifyContent: 'center',
						backgroundColor: urlData.videoUrl
							? 'transparent'
				...
				>
					...
					>
						...
						<div style={{ marginBottom: 16 }}>
							<Typography
								sx={{ fontSize: 14, color: '#374151', marginBottom: 8 }}
							>
								Videos guardados
							</Typography>
							{videos.length === 0 ? (
									...
									<Select
										...
										onChange={e => {
											...
											setFormData(prev => ({
												...
												activo: tipo === 'en_directo', // 👈 regla solicitada
											}));
										}}
										size='small'
									>
