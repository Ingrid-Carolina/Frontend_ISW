...
import {
  ...
  Menu, MenuItem, Checkbox, FormControlLabel, Tabs, Tab,
  Select, FormControl, InputLabel, Chip, CircularProgress, Alert
} from "@mui/material";
...
const radarConfigs = {
  ofensivo: {
    ...
    maxValues: { AVG: 0.400, HR: 50, RBI: 150, SLG: 0.700, OPS: 1.200, OBP: 0.500 }
  },
  defensivo: {
  ...
  avanzado: {
    ...
    maxValues: { GPA: 0.350, RC: 120, TB: 350, XBH: 80, "BB/K": 2.0, OBP: 0.500 }
  }
};
...
export default function StatsJugadores() {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        data.forEach(row => {
          ...
          const conflictosNombre = jugadoresTemp.filter(j => 
            j.name.toLowerCase() === nuevoJugador.name.toLowerCase()
          );
        ...
        if (sel.actualizar) {
          ...
          if (exito) {
            jugadoresTemp = jugadoresTemp.map(j =>
              j.ID === conflicto.conflictosExistentes[0].ID ? jugadorActualizado : j
            );
          }
        } else if (sel.agregar) {
  ...
  return (
    <Box sx={{ width: "100%", pt: 2 }}>
      <Typography variant="h2" sx={{ fontFamily: "GroteskBold", color: "#10045c", mb: 4, textAlign: "center" }}>
      ...
      {error && (
        <Alert severity="error" sx={{ mb: 3, maxWidth: 600, mx: "auto" }}>
          {error}
          <Button onClick={fetchJugadores} sx={{ ml: 2 }}>Reintentar</Button>
        </Alert>
      )}

      <Box sx={{ textAlign: "center", mb: 3 }}>
        <Button variant="contained" component="label" sx={{ backgroundColor: "#10045c", mr: 2 }}>
          Importar Excel con Estadísticas
          <input type="file" accept=".xlsx, .xls" hidden onChange={handleImportExcel} />
        </Button>
        <Button variant="outlined" onClick={fetchJugadores} disabled={loading}>
          ...
          <Grid item xs={12} sm={6} md={4} key={jugador.ID}>
            <Card sx={{ borderRadius: 3, boxShadow: 3, textAlign: "center", p: 2, position: "relative" }}>
              ...
              <CardMedia
                ...
                image={jugador.foto}
                alt={jugador.name}
                sx={{ objectFit: "cover", borderRadius: 3, maxWidth: "300px", mx: "auto", mb: 2 }}
              />
              <CardContent>
                <Typography variant="h6" sx={{ fontFamily: "GroteskBold", mb: 1 }}>
                  {jugador.name}
                </Typography>
            ...
            >
              <Box>
                ...
                <Typography variant="body2" color="textSecondary">
                  Conflicta con: {conflicto.conflictosExistentes.map(j => `${j.name} (ID: ${j.ID})`).join(", ")}
                </Typography>
              </Box>
      ...
      <Dialog open={openConfirmarSinSeleccion} onClose={() => setOpenConfirmarSinSeleccion(false)} maxWidth="sm" fullWidth>
        <DialogContent sx={{ textAlign: "center", p: 3 }}>
          ...
          <Box sx={{ mt: 3, display: "flex", justifyContent: "center", gap: 2 }}>
            ...
            <Button variant="contained" onClick={aplicarCambios} disabled={loading}>
              {loading ? <CircularProgress size={20} /> : "Continuar de todos modos"}
            </Button>
          </Box>
        </DialogContent>
      </Dialog>
          ...
          {jugadorSeleccionado && (
            <Box sx={{ textAlign: "center", p: 2 }}>
                ...
                <Box sx={{ mb: 3 }}>
                  <FormControl sx={{ minWidth: 200, mb: 3 }}>
                    ...
                    >
                      ...
                      <MenuItem value="defensivo">Estadísticas de Contacto</MenuItem>
                      <MenuItem value="avanzado">Métricas Avanzadas</MenuItem>
                    </Select>
                  </FormControl>

                  <Card sx={{ borderRadius: 4, boxShadow: 6, p: 3, background: 'linear-gradient(135deg, #667eea 0%, #10045c 100%)', mb: 3 }}>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <RadarChart data={prepararDatosRadar(jugadorSeleccionado, radarConfig)}>
                        <PolarGrid gridType="polygon" />
                        <PolarAngleAxis dataKey="stat" tick={{ fontSize: 12, fill: 'white' }} />
                        <PolarRadiusAxis
                          angle={90}
                          domain={[0, 'dataMax']}
              ...
              {modoDetalle === "detallado" && (
                <Card sx={{ borderRadius: 4, boxShadow: 6, p: 3, background: '#10045c', maxWidth: "700px", mx: "auto", mb: 2 }}>
                  <Box sx={{ columnCount: 3, columnGap: "20px", px: 3 }}>
                    {orderStats.filter(statKey => !["name", "IMG"].includes(statKey)).map(statKey =>
                      jugadorSeleccionado[statKey] !== undefined && jugadorSeleccionado[statKey] !== null && (
              ...
              {modoDetalle === "resumido" && (
                <Card sx={{ borderRadius: 4, boxShadow: 6, p: 3, background: '#10045c', maxWidth: "700px", mx: "auto", mb: 2 }}>
                  {Object.entries(resumenStats).map(([key, label]) =>
                    jugadorSeleccionado[key] !== undefined && jugadorSeleccionado[key] !== null && (
                      <Typography key={key} sx={{ mb: 1, fontSize: "16px", color: "#ffffffff" }}>
                        <b>{label} ({key}):</b> {jugadorSeleccionado[key]}
                      </Typography>
                    )
                  )}
                </Card>
              ...
              {tabValue === 0 && (
                <Box>
                  ...
                  <Card sx={{ 
                    ...
                    boxShadow: 6, 
                    p: 3, 
                    background: 'linear-gradient(135deg, #667eea 0%, #10045c 100%)', 
                    mb: 3 
                  }}>
                    <Typography variant="h6" sx={{ color: "white", mb: 2 }}>
                    ...
                    <Box sx={{ display: "flex", justifyContent: "center", gap: 4, mb: 2 }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        ...
                        <Typography sx={{ color: "white", fontSize: "14px" }}>{jugadorBase.name}</Typography>
                      </Box>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Box sx={{ width: 20, height: 20, backgroundColor: "#4ecdc4", borderRadius: 1 }} />
                        <Typography sx={{ color: "white", fontSize: "14px" }}>{jugadorComparado.name}</Typography>
                      </Box>
                    </Box>

                    <ResponsiveContainer width="100%" height={400}>
                      <RadarChart data={prepararDatosComparacion(jugadorBase, jugadorComparado, radarConfig)}>
                        <PolarGrid gridType="polygon" />
                        <PolarAngleAxis dataKey="stat" tick={{ fontSize: 12, fill: 'white' }} />
                        <PolarRadiusAxis
                          angle={90}
                      ...
                      <React.Fragment key={statKey}>
                        <Typography
                          sx={{ 
                            ...
                            fontWeight: parseFloat(jugadorBase[statKey]) > parseFloat(jugadorComparado[statKey]) ? "bold" : "normal",
                            color: parseFloat(jugadorBase[statKey]) > parseFloat(jugadorComparado[statKey]) ? "#10045c" : "inherit"
                          }}
                        >
                          {jugadorBase[statKey]}
                        </Typography>
                        ...
                        <Typography
                          sx={{ 
                            py: 1, 
                            background: index % 2 === 0 ? "#f9f9f9" : "white",
                            fontWeight: parseFloat(jugadorComparado[statKey]) > parseFloat(jugadorBase[statKey]) ? "bold" : "normal",
                            color: parseFloat(jugadorComparado[statKey]) > parseFloat(jugadorBase[statKey]) ? "#10045c" : "inherit"
                          }}
                        >
