...
// Importa XLSX para procesar archivos Excel
import * as XLSX from 'xlsx';
// Importa el cliente API personalizado
import { api } from '../../api/api';
// Importa componentes de Recharts para gráficos radar
import {
    RadarChart,
    Radar,
    PolarGrid,
...
// Importa íconos para acciones
import MoreVertIcon from '@mui/icons-material/MoreVert';
import BarChartIcon from '@mui/icons-material/BarChart';
import RadarIcon from '@mui/icons-material/Radar';
import CompareArrowsIcon from '@mui/icons-material/CompareArrows';

// --- Estadísticas ordenadas para mostrar y comparar ---
const orderStats = [
...
// --- Estadísticas resumidas para vista rápida ---
const resumenStats = {
    G: 'Juegos',
...
export default function StatsJugadores() {
    // --- Estados principales ---
    const [jugadoresExcel, setJugadoresExcel] = useState([]); // Jugadores importados desde Excel
    const [jugadores, setJugadores] = useState([]); // Jugadores en el sistema
    const [open, setOpen] = useState(false); // Modal de detalle jugador
    const [jugadorSeleccionado, setJugadorSeleccionado] = useState(null); // Jugador seleccionado para ver detalle
    const [modoDetalle, setModoDetalle] = useState('detallado'); // Modo de vista en el modal
    const [busqueda, setBusqueda] = useState(''); // Filtro de búsqueda por nombre
    const [tabValue, setTabValue] = useState(0); // Tab activo en comparación
    const [radarConfig, setRadarConfig] = useState('ofensivo'); // Tipo de gráfico radar
    const [loading, setLoading] = useState(true); // Estado de carga
    const [error, setError] = useState(null); // Estado de error

    // --- Estados para menú y comparación ---
    const [anchorEl, setAnchorEl] = useState(null); // Anchor para menú de acciones
    const [jugadorMenu, setJugadorMenu] = useState(null); // Jugador en menú

    const [openComparar, setOpenComparar] = useState(false); // Modal de comparación
    const [jugadorBase, setJugadorBase] = useState(null); // Jugador base para comparar
    const [jugadorComparado, setJugadorComparado] = useState(null); // Jugador comparado

    // --- Estados para conflictos de importación ---
    const [jugadorEnConflicto, setJugadorEnConflicto] = useState([]); // Conflictos de nombre al importar
    const [openConflicto, setOpenConflicto] = useState(false); // Modal de conflictos
    const [seleccionConflictos, setSeleccionConflictos] = useState({}); // Resolución de conflictos

    const [openConfirmarSinSeleccion, setOpenConfirmarSinSeleccion] = useState(false); // Modal de confirmación sin selección
    const [jugadoresSinSeleccion, setJugadoresSinSeleccion] = useState([]); // Jugadores sin acción seleccionada

    // --- Estado para notificaciones tipo snackbar ---
    ...
    const fetchJugadores = async () => {
        try {
            ...
            // Mapea los datos de la BD a las claves del frontend
    ...
    // --- Aplica los cambios después de resolver conflictos de importación ---
    ...
    // --- Aplica los cambios de conflictos, validando que todos tengan acción seleccionada ---
    ...
    // --- Prepara datos para gráfico radar de un jugador ---
    ...
    // --- Prepara datos para comparar dos jugadores en gráfico radar ---
    ...
    return (
        <Box sx={{ width: '100%'}}>
            <Toolbar /> {/* Espaciador para la barra de navegación fija */}
            <Box sx={{ pt: 3}}>
                <Typography
                    variant="h2"
                    sx={{
                ...
                {jugadoresFiltrados
                    ...
                    .map(jugador => (
                        <Grid item xs={12} sm={6} md={4} key={jugador.ID}>
                            ...
                            >
                                {/* Menú de acciones para cada jugador */}
                                <IconButton
            ...
            {/* Menú de acciones para comparar jugador */}
            <Menu
                anchorEl={anchorEl}
            ...
            {/* Popup para resolver conflictos de importación */}
            <Dialog
                open={openConflicto}
            ...
            {/* Popup confirmación si hay jugadores sin acción seleccionada */}
            <Dialog
                ...
                <DialogContent>
                    {jugadorSeleccionado && (
                        <Box sx={{ textAlign: 'center', p: 2 }}>
                            ...
                            {/* Vista resumida de estadísticas */}
            ...
            {/* Modal para comparar dos jugadores */}
            <Dialog
                open={openComparar}
                onClose={() => {
            ...
            >
                ...
                >
                    {/* Selección de jugador para comparar */}
                    {!jugadorComparado ? (
                        <Box sx={{ textAlign: "center", p: 2 }}>
                            <Typography
                                variant="h6"
                        ...
                        <Box sx={{ textAlign: 'center' }}>
                            ...
                            {/* Tabs para cambiar entre gráfico radar y tabla comparativa */}
                            <Tabs
                                value={tabValue}
